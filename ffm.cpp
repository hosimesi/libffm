...
X, P: Used to store the problem in a compressed sparse row (CSR) format. len(X)
= nnz, len(P) = l + 1 Y: The label. len(Y) = l R: Precomputed scaling factor to
make the 2-norm of each instance to be 1. len(R) = l v: Value of each element in
the problem
*/
...
void ffm_read_problem_to_disk(string txt_path, string bin_path) {
  ...
  if (same_file) {
    ...
    cout << "(" << fixed << setprecision(1) << timer.toc() << " seconds)"
         << endl;
  }
}
...
void ffm_save_model(ffm_model &model, string path) {
  ...
  // Need to write chunk by chunk because some compiler use int32 and will
  // overflow when w_size * 4 > MAX_INT

  for (ffm_long offset = 0; offset < w_size;) {
    ffm_long next_offset =
        min(w_size, offset + (ffm_long)sizeof(ffm_float) * kCHUNK_SIZE);
...
ffm_model ffm_load_model(string path) {
  ...
  for (ffm_long offset = 0; offset < w_size;) {
    ffm_long next_offset =
        min(w_size, offset + (ffm_long)sizeof(ffm_float) * kCHUNK_SIZE);
    ffm_long size = next_offset - offset;
    f_in.read(reinterpret_cast<char *>(model.W + offset),
              sizeof(ffm_float) * size);
    offset = next_offset;
  }
